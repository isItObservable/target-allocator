...
sed -i "s,CLUSTER_NAME_TO_REPLACE,$CLUSTERNAME,"  opentelemetry/openTelemetry-manifest_debut.yaml
sed -i "s,CLUSTER_NAME_TO_REPLACE,$CLUSTERNAME,"  opentelemetry/openTelemetry-manifest_statefulset.yaml
kubectl apply -f dynatrace/dynakube.yaml -n dynatrace
# Deploy collector
kubectl create secret generic dynatrace  --from-literal=dynatrace_oltp_url="$DTURL" --from-literal=clusterid=$CLUSTERID  --from-literal=dt_api_token="$DTTOKEN"
